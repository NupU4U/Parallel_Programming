...
void getMaxProbabilityIndex(const std::string& filenaame, float* convMatrix, float* convMatrix2, float* convMatrix3, float* convMatrix4, float* bias1,float* bias2, float* bias3, float* bias4, std::pair<float, int>* top5){
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (entry->d_type == DT_REG) { // Check if it's a regular file
                ...
                if (file.is_open()) {
                    ...
                    getMaxProbabilityIndex(folderPath + "/" + filename,  convMatrix, convMatrix2, convMatrix3, convMatrix4, bias1, bias2, bias3, bias4,top5ValuesWithIndices);
